...
public class DataSource_doc
{
     ...
     * <td>
     * 
     * 数据源的开始与数据源管理接口服务 {@link com.solmix.api.datasource.DataSourceManagerService DataSourceManagerService} 默认的接口实现为
     * {@link com.solmix.fmk.datasource.DataSourceManager DataSourceManager}. 通过数据源管理服务的方法
     * {@link com.solmix.fmk.datasource.DataSourceManager#get(String) get()} 获取数据源。在生成环境下，数据源从通过数据源池
     * {@link com.solmix.fmk.pool.PoolManager PoolManager} 中获取，PoolManager 是一个池管理器，通过实现{@link com.solmix.api.pool}
     * 下定义的接口来完成具体的池管理功能。数据源管理池的默认实现是 {@link com.solmix.fmk.datasource.PoolableDataSourceFactory
     * PoolableDataSourceFactory}。池管理器第一次通过调用
     * {@link com.solmix.api.pool.SlxKeyedPoolableObjectFactory#makeUnpooledObject(Object) makeUnpooledObject}
     * 往数据源池中加载对象。{@link com.solmix.fmk.datasource.DataSourceProvider#forName(String) DataSourceProvider.forName}
     * 的任务就是为其提供数据源。 数据源的初始化通过三个步骤完成：
     * <li>前置处理器：加载和初始化配置
     * <li>自定义处理器：根据各个数据源的特性，对数据做处理
     * <li>后置处理器：对所有的数据进行统一处理，以提供一个可用的数据源配置集合。 {@link com.solmix.fmk.datasource.DataSourceProvider#forName(String)}
     * responsible to provided datasource Before {@link com.solmix.api.datasource.DataSource#init(DataSourceData)}
     * parser the xml-format data by {@link com.solmix.api.datasource.ParserHandler}</td>
     * </tr>
     * <tr>
     * <td COLSPAN="2" width="100%" align="left" style="background-color : #BBFFFF;font-size:12pt">
     * <li>各种数据源可用的数据项定义</td>
     ...
     * <tr>
     * <td >通用</td>
     * <td>{@link com.solmix.api.jaxb.TdataSource}</td>
     * </tr>
     * <tr>
