...
import com.solmix.api.context.Context.Scope;
...
import com.solmix.fmk.rpc.Transaction;
...
public final class SlxContext
{
    ...
    public static SecurityAdmin getSecurityAdmin() {
        AbstractSystemContext abc = getAbstractSystemContext();
        if (abc != null) {
            SecurityAdmin dsm = abc.getSecurityAdmin();
            if (dsm == null) {
                log.debug("SecurityAdmin is not set.");
            }
            return dsm;
        } else {
            return null;
    ...
    public static void login(Subject subject) {
        if (getContext() instanceof UserContext) {
            ((UserContext) getContext()).login(subject);
        } else {
            throw new IllegalArgumentException("This context  is not a user context,can't use login method.");
        }
    }
    ...
    /**
     * Executes the given operation in the web context ,if this thread is not has a {@link com.solmix.api.context.WebContext} in it,
     * return null.
     * @param op
