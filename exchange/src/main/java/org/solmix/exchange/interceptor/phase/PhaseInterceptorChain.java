...
public class PhaseInterceptorChain implements InterceptorChain {
    ...
    /**
     * heads[phase]为该阶段拦截器的第一个节点 heads[phase]
     * 和tails[phase]直接的节点为该阶段(phase)所有的拦截器
     */
        ...
        if (heads[phase] == null) {
            ...
            if (hasAfters[phase] || !before.isEmpty()) {
                ...
                while (ih2 != tails[phase].next) {
                    ...
                    if (cmpId != null
                        && firstBefore == null
                        && (before.contains(cmpId) || cmp.getAfter().contains(
                            id))) {
                        firstBefore = ih2;
                    }
            ...
            if (firstBefore == null && lastAfter == null && !before.isEmpty()
                && after.isEmpty()) {
     ...
     * {@inheritDoc}
     * 
     * @see org.solmix.exchange.interceptor.InterceptorChain#remove(org.solmix.exchange.interceptor.Interceptor)
     */
    @Override
    ...
    public boolean doIntercept(Message message) {
        ...
        try {
            ...
            if (oldMessage != null && !message.containsKey(PREVIOUS_MESSAGE)
                ...
                message.put(PREVIOUS_MESSAGE, new WeakReference<Message>(
                    oldMessage));
            }
                    ...
                    if (!faultOccurred) {
                        ...
                        if (message.getExchange() != null) {
                            ...
                            isOneWay = message.getExchange().isOneWay()
                                && MessageUtils.getBoolean(message,
                                    Message.ONEWAY);
                        }
    ...
    public boolean doInterceptAfter(Message message, String inteceptorId) {
        updateIterator();
        while (state == State.EXECUTING && iterator.hasNext()) {
            PhaseInterceptor<? extends Message> currentInterceptor = (PhaseInterceptor<? extends Message>) iterator.next();
     ...
     * {@inheritDoc}
     * 
     * @see org.solmix.exchange.interceptor.InterceptorChain#listIterator()
     */
    @Override
    ...
    /** 内部迭代器 */
    static final class PhaseIterator implements
     ...
     * {@inheritDoc}
     * 
     * @see org.solmix.exchange.interceptor.InterceptorChain#getFaultProcessor()
     */
    @Override
