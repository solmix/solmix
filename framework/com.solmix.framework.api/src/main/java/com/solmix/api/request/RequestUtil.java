...
public class RequestUtil
{
     ...
     * 
     * @param value
     * @return A Map indexed by the Token names where the values are Map instances indexed by parameter name
     */
     ...
     * 
     * @param value
     * @return A Map indexed by the Token names where the values are <code>Double</code> instances providing the value
    ...
    /**
     * Utility method to return a name for the given servlet. This method applies the following algorithm to find a non-
     * <code>null</code>, non-empty name:
     * <ol>
     * <li>If the servlet has a servlet config, the servlet name from the servlet config is taken.
     ...
     * @param value The new value of the attribute. If this is <code>null</code> the attribte is actually removed from
     *        the request.
     * @return The previous value of the named request attribute or <code>null</code> if it was not set.
     */
    ...
    /**
     * Checks if the request contains a if-last-modified-since header and if the the request's underlying resource has a
     * jcr:lastModified property. if the properties were modified before the header a 304 is sent otherwise the response
     ...
     */
    public static boolean handleIfModifiedSince(SlxHttpServletRequest req, HttpServletResponse resp) {
        boolean responseSet = false;
        long lastModified = -1;
        // req.getResource().getResourceMetadata().getModificationTime();
        if (lastModified != -1) {
