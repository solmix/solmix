 ...
 * 
 * @author solomon
 * @version $Id$  2011-11-16
 */

public class IMCometHandler extends AtmosphereGwtHandler
{
    private ConcurrentHashMap<Integer,String > connetionIdCache= new ConcurrentHashMap<Integer,String>();
    @Override
    ...
    public int doComet(GwtAtmosphereResource resource) throws ServletException, IOException {
        resource.getBroadcaster().setID("IMCOMET");
        ...
        connetionIdCache.put( resource.getConnectionID(),currentUser);
        return NO_TIMEOUT;
    }
     ...
     * @param message
     */
    private void processIMEvent(IMEvent event) {
        String targetName =event.getTargetName();
        if(targetName==null)
            return;
        for(int id:connetionIdCache.keySet()){
           String name = connetionIdCache.get(id);
           if(targetName.equals(name)){
               GwtAtmosphereResource resource=  lookupResource(id);
               if(resource==null){
                   connetionIdCache.remove(id);
                   continue;
               }
               broadcastTo(event, resource);
           }
        }
     ...
     * @param resource
     */
    private void broadcastTo(IMEvent event, GwtAtmosphereResource resource) {
        if (event == null) {
            return;
        }
        getBroadcaster(resource).broadcast(event, resource.getAtmosphereResource());
