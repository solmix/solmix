...
public class ResourceBundleCache
{
    ...
    ResourceBundle getResourceBundle(Locale locale) throws I18nException {
        ...
        try {
            rb = getResourceBundleInternal(locale == null ? ResourceBundleManager.DEFAULT_LOCALE : locale);
            if (rb == null)
                rb = getResourceBundleInternal(ResourceBundleManager.DEFAULT_LOCALE);
        } catch (IOException e) {
            e.printStackTrace();
            throw new I18nException("Exception When load properties", e);
        }
        return rb;
                ...
                if (locales != null) {
                    while (locales.hasMoreElements()) {
                        ...
                        if (entryPath.length() == 0) {
                            entryPath = "_" + ResourceBundleManager.DEFAULT_LOCALE.toString();
                        }
