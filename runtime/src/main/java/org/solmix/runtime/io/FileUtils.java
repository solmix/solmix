...
public final class FileUtils {
    ...
    public static File createTmpDir() {
        ...
        if (checkExists.getUsableSpace() < 1024 * 1024) {
           LoggerFactory.getLogger(FileUtils.class).warn("The directory " + s + " has very "
                                                           + "little usable temporary space.  Operations"
                                                           + " requiring temporary files may fail.");
        }
        File f = new File(checkExists, "solmix-tmp-" + x);
        int count = 0;
        while (!f.mkdir()) {
            ...
            f = new File(checkExists, "solmix-tmp-" + x);
            count++;
        }
        File newTmpDir  = f;
        final File f2 = f;
